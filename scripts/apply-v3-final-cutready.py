...
patch_html('v3-preview.html')

css = read('css/v3-preview.css')
css += '''
...
'''
app = replace_once(app, "  function buildCalibrationSvg(){", cut_ready_functions + "\n  function buildCalibrationSvg(){", 'cut-ready functions')

render_pattern = re.compile(r"  function renderPreflight\(\)\{.*?\n  \}\n  function openPreflight", re.S)
render_replacement = r'''  function renderPreflight(){
...
readme = re.sub(r"## สิ่งที่ตั้งใจไม่ทำแบบเดาสุ่ม\n\n.*?\n\n## Verification อัตโนมัติ",
'''## Export Final
...
## Verification อัตโนมัติ''', readme, count=1, flags=re.S)
readme = replace_once(readme,
...
print('Applied final Cut Ready Curve/Path implementation.')
