(() => {
  ...
  function initUx(){initTopMenus();initPanelResize();initNavigation();initFontPicker();initCanvasMenus();refreshRestoreAvailability();}
  ...
  function frameMarkup(o,p,interactive=true){const t=transformFor(p,o);const visible=`<rect ${interactive?`class="frame-shape" data-object="${o.id}" data-placement="${p.id}"`:''} x="${t.x}" y="${t.y}" width="${o.size.w}" height="${o.size.h}" ${interactive?'':'fill="none" stroke="#000" stroke-width="0.3"'}/>`;const hit=interactive?`<rect class="frame-hit" data-object="${o.id}" data-placement="${p.id}" x="${t.x}" y="${t.y}" width="${o.size.w}" height="${o.size.h}"/>`:'';return rotGroup(visible+hit,t.rotation,t.x+o.size.w/2,t.y+o.size.h/2);}
  ...
  const hadAutosave=!!storageGet(AUTOSAVE_KEY);state.activeDesignId=null;state.selected={placementId:null,objectId:null};state.selection=[];ensureWorkspaceState();setTab('layers');syncUnitButtons();renderAll();state.persistence.ready=true;initUx();if(hadAutosave){setAutosaveStatus('มีงานล่าสุดที่กู้คืนได้','saved');toast('มีงานล่าสุดที่กู้คืนได้จากเมนู “ไฟล์”');}else{setAutosaveStatus('บันทึกอัตโนมัติเปิดอยู่','saved');scheduleAutosave(true);}centerCanvasView();
